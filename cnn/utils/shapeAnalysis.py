...
import numpy as np
import torch 
def getPseudoKernel(num_hid_layer:int, layer_design:dict)->tuple[int, int]:
    '''
    ...
    Returns:
    p_pseudo, r_pseudo : (int, int) 
        effective convolution kernel dimension in PE - and RO-direction, respectively.
    '''    
...
def extractDatCNN(acs:torch.Tensor, R:int, num_hid_layer:int, layer_design:dict)->dict:
    '''
    Function to get Training-Data for CNNs for k-space interpolation. 
    Args: 
    acs : torch.Tensor
        Auto-Calibration-Signal in shape [coil, PE, RO].
    R : int
        Undersampling Factor.
    num_hid_layer : int
        Number of 'hidden' layers in Network. 
    layer_design : dict
        Dictionary describing Network-Architecture.

    Returns:
    prc_data : dict
        Dictionary with keys 'src': Source-Signals.
                                        'trg': Target-Signals.
    '''    
    ...
    dim_trg_vec = (R) * acs.shape[0]

    # initializing the source - and target - matrices obtained from acs-block
    trg_data = torch.zeros((rep_p_acs, rep_r_acs, dim_trg_vec),
                          dtype=acs.dtype)
    ...
    for p in range(rep_p_acs):
        ...
        for r in range(rep_r_acs):
            trg_data[p, r, ] = acs[:, p+p_trg*R+1:p+p_trg*R+R+1:1, r+r_trg
                                  ].reshape((dim_trg_vec))

    trg_data = torch.unsqueeze(trg_data, 1)

    src_data = torch.zeros((rep_p_acs,
                           num_coils,
                           kernel_extension_p,
                           num_r_acs),
                           dtype=acs.dtype
    ...
    src_data = src_data.permute((0, 2, 3, 1))
...
def fillDatCNN(kspace_zf:torch.Tensor, pred_mat:torch.Tensor, R:int, num_hid_layer:int, layer_design:dict)->torch.Tensor:
    '''
    ...
    for p in range(rep_p):
        for r in range(rep_r):
            kspace_zf[:, p*R+p_trg*R+1:p*R+p_trg*R +
                 R+1:1, r+r_trg] = pred_mat[R*p, r, ].reshape((num_coil, R, ))
